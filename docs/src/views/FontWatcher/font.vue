...
const createFontLink = (url: string) => store.fontLink(store.config.root + url)
</script>

<template>
    <div v-for="(item) in store.fontDetail!.css || []" :key="item.fontFamily"
        ...
        <!--  字体信息 -->
        <div class="font-light w-full flex justify-start items-baseline text-sm pb-1 mb-1 divide-x-2 italic">
        ...
        <div class="relative flex items-center pt-4 pb-2">
            <!-- 字符展示 -->
            <div class="text-2xl text-left" :style="{
                fontFamily: item.fontFamily,
                fontWeight: item.fontWeight || 'normal'
            }">
                {{ store.config.test }}
                <link rel="stylesheet" :href="createFontLink(item.url)" />
            </div>
            <!-- 选择按钮 -->
            <div class="absolute right-0 top-0 h-full w-1/2 flex flex-row-reverse items-center">
                <div class="w-5/6 pr-4 from-transparent to-white bg-gradient-to-r float-right">
                    <div class="text-lg font-bold float-right px-2 py-2 rounded-md text-teal-400 h-fit cursor-pointer hover:bg-teal-100/50 flex items-center"
